...
const getUserChallengesController = async (req, res) => {
  try {
    const { userId } = req.params;
    const challenges = await getChallenges([Query.equal("creatorId", userId)]);
    res.status(200).json(challenges);
  } catch (error) {
    res
      .status(500)
      .json({ error: "Failed to fetch user challenges", details: error.message });
  }
};
