...
const fetchCommentsWithReplies = async (challengeId, parentCommentId = null) => {
  const comments = await getComments(challengeId, parentCommentId);
  return await Promise.all(
    comments.map(async (comment) => {
      const replies = await fetchCommentsWithReplies(challengeId, comment.$id); // Recursive call
      return { ...comment, replies };
    })
  );
};
...
const listComments = async (req, res) => {
  try {
    ...
    const commentsWithReplies = await fetchCommentsWithReplies(challengeId);
    res.json(commentsWithReplies);
  } catch (error) {
    console.error('Error in listComments:', error);
    res.status(500).json({ error: 'Failed to fetch comments', details: error.message });
